...
In this paper, we attempts to replicate the Bogdonova et al. paper \cite{bogdanova2015detecting} that propose a method for identifying semantically equivalent questions based on a convolutional neural network. 
We solve the problem of unequal text legths by considering the \emph{max} length, which reaches better performance with respect to the mean approach. We experimentally show that the proposed CNN achieves high accuracy especially when the word embeddings are pre-trained on in-domain data. In particular we found out that the CNN Multi-channel model brings a performance improvement of one-percent with respect to the model with only one matrix of word embeddings . This model achieves 89\% accuracy on the test set, that is a result very close to the one of our target.\\
